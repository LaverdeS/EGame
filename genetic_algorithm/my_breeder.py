...
class Breeder:
    ...
    def initialize_population_function(self, num_individuals, color):
        ...
        for _ in range(num_individuals):
            population.append(Dot(self.parent, color=color)) #Here I could just create random dna for num_individuals
        return population
    ...
    def assess_individual_fitness(self, individual):
        ...
        #score is a matrix rows 3= column 6 or around
        #score = (dna[0][0] + dna[0][4] + dna[1][0] + dna[1][5] + dna[2][0]) * (statistic.food_seen + statistic.food_eaten)
        #score = dna[0][3] + dna[1][3] + dna[2][2] + statistic.enemies_attacked + statistic.consumed_corpses + statistic.opponents_seen
        score = statistic.time_survived * statistic.food_eaten
        return score
    
    # Here I have my first opportunity to change something: FITNESS FUNCTION
    ...
    #Then change the select function
    ...
    def crossover(parents):
        ...
        for i in range(len(parents)-1):
            child_a = copy(parents[i])
            j = i + 1
            while j < len(parents) and counter <= population_size:
                child_b = copy(parents[j])

                k = random.randint(1, len(features)-1) #one point crossover
                #swap with separator at k
                temp = copy(child_b[0:k])
                child_b[0:k] = copy(child_a[0:k])
                child_a[0:k] = copy(temp)
    ...
    #return np.copy(np.array(configuration))
    ...
    def tweak_example(self, individual):
        ...
        perc = self.mutate_dna(dna=perc, increase_value=increase, increase=0)
        des = self.mutate_dna(dna=des, increase_value=increase, increase=0)
        abil = self.mutate_dna(dna=abil, increase_value=increase, increase=0)
